...
continue}else if(typeA===types.FUNCTION&&typeB===types.FUNCTION){var _A=objectA[property].toString();var _B=objectB[property].toString();if(_A!==_B){diffResponse.aggregator(new propertyData(parent,property,_A,_B,valueStatus.MODIFIED))}}else if(objectA[property]!==objectB[property]){diffResponse.aggregator(new propertyData(parent,property,objectA[property],objectB[property],valueStatus.MODIFIED))}}else{diffResponse.aggregator(new propertyData(parent,property,objectA[property],null,valueStatus.DELETED))}}}
...
function Configuration(_config){this.deep=ifNotDefinedGetDefault(_config,'deep',3);this.scan=ifNotDefinedGetDefault(_config,'scan',{arrays:!0});this.returnType=ifNotDefinedGetDefault(_config,'returnType',returnTypes.JSON_OBJECT)}
...
var parent='$root';var diffResponse=DiffResponseFactory();diffResponse.caller=this;if(config.returnType===returnTypes.JSON_OBJECT){var aggregator=function aggregator(data){if(data.property!==null){this.data[data.property]=data}else{this.data=data}};var clearData=function clearData(){this.data={}};diffResponse.aggregator=aggregator;diffResponse.clearData=clearData;diffResponse.clearData()}else{var _aggregator=function _aggregator(data){this.data.push(data)};var _clearData=function _clearData(){this.data=[]};diffResponse.aggregator=_aggregator;diffResponse.clearData=_clearData;diffResponse.clearData()}
